...
/*   Updated: 2026/06/18 14:50:44 by fmotte           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
AMethod *CGI::getAmethod() const
{
    return _amethod;
}

void CGI::setAmethod(AMethod *amethod)
{
    if (amethod == NULL)
        throw ExecptionErrorUninitializedVariable("*amethod", "CGI");
...
void CGI::setPipeIn(int pipeIn[2])
{
    _pipeIn[0] = pipeIn[0];
    _pipeIn[1] = pipeIn[1];
}
...
void CGI::setPipeOut(int pipeOut[2])
{
...
// CGI pipeIn[1] à fermer sinon le serveur attendra indéfiniment (deadlock)

void CGI::createPipe(int pipeIn[2], int pipeOut[2])
{
...
void CGI::checkForkCreate(pid_t pid)
{
    if (pid == -1)
    {
        std::cerr << "Error fork\n";
...
void CGI::initializationCGI(const std::string &path, const std::string &interpreter)
{
    int pipeIn[2];
    int pipeOut[2];
    ...
    if (pid == 0)
        manage_pipe(path, getPipeIn(), getPipeOut(), interpreter);
        
    close(getPipeOut()[1]);
}

void CGI::connectToEpoll()
{
    int epoll_fd = getAmethod()->getHttpRequest()->getRequest()->getClient()->getWebserv()->getEpollFd();

    addFdToEvent(epoll_fd, getPipeIn()[1], EPOLLOUT);
    addFdToEvent(epoll_fd, getPipeOut()[0], EPOLLIN);
}

void CGI::manage_pipe(const std::string &path, int pipeIn[2], int pipeOut[2], const std::string &interpreter)
{
    close(pipeOut[0]);
    ...
    std::string uri = getAmethod()->getHttpRequest()->getHeader()->getUri();
    std::string query = "";
    ...
    std::string method = (getAmethod()->getHttpRequest()->getHeader()->getMethod() == POST) ? "POST" : "GET";
    std::string protocol = getAmethod()->getHttpRequest()->getHeader()->getProtocol();
    
    HeaderContent hc = getAmethod()->getHttpRequest()->getHeader()->getHeaderContent();
    ...
    Listen *listen = getAmethod()->getHttpRequest()->getRequest()->getServer()->getListen(0);
